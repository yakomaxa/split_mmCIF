...
function(pdb.files, ids=NULL, path="split_chain", overwrite=TRUE, verbose=FALSE, mk4=FALSE, ncore=1, progress=NULL, ...) {
  ...
  "splitOnePdb" <- function(i, pdb.files, ids, path, overwrite, verbose, ...) {
    ...
    if(!overwrite && !verbose) {
      ...
      invisible(capture.output( pdb <- read.cif(pdb.files[i], verbose=verbose, ...) ))
      chains <- unique(pdb$atom[, "chain"])
    }
    else {
      pdb <- read.cif(pdb.files[i], verbose=verbose, ...)
      chains <- unique(pdb$atom[, "chain"])
    }
...
args=commandArgs(T)
#split model 1 (posed to 4un3 by mican)
pdb.ids <- args[2:(length(args[]))]
outdir = args[1]
raw.files <- get.pdb(pdb.ids,path=outdir,format="cif")
all.files <- cifsplit(raw.files,pdb.ids,path=paste(outdir,"/split_chain",sep=""))
